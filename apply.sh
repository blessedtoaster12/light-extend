...
original_author="blessedtoaster12"
original_name="light_extend"
original_urlname="light-extend"
original_description="Awesome light_extend created by blessedtoaster12" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/light_extend "${name}"
...
then
    ...
    echo "then run 'light_extend' or 'python -m light_extend'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'light_extend' or 'python -m light_extend' or 'poetry run light_extend'"
fi
