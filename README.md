...
- 🎯 Entry points to execute your program using `python -m <light_extend>` or `$ light_extend` with basic CLI argument parsing.
...
---
# light_extend Flask Application

Awesome light_extend created by blessedtoaster12
...
```bash
git clone https://github.com/blessedtoaster12/light-extend light_extend
cd light_extend
make install
```
...
```bash
light_extend create-db   # run once
light_extend populate-db  # run once (optional)
light_extend add-user -u admin -p 1234  # ads a user
light_extend run
```
